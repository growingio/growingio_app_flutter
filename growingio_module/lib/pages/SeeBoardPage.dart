...
class _SeeBoardPage extends State<SeeBoardPage> {
  ...
  Widget getChartView(int index) {
    ...
    datatime = 0;
    for (int i = (arr.length / 2).toInt();
        i < arr.length;
        i += (arr.length / 2 / 24).toInt()) {
  ...
  List<Widget> _getwidgget(List arr, String name, String str, int index) {
    ...
    lview.add(Expanded(child: _simpleLine(index)));
    lview.add(SizedBox(
      height: 5,
    ));
    if (arr.length > 20) {
      lview.add(Center(
        ...
        children: <Widget>[
          ...
          Text(gettimeStampWithTime(
              arr[(arr.length - 2).toInt()][0].toString())), //
        ],
      )));
    } else {
      lview.add(Center(
  ...
  List<Widget> _listViewData() {
    ...
    for (int i = 0; i < SummaryPageData.chartdata.length; i++) {
      ...
      list.add(Container(
          ...
          child: Column(children: _getwidgget(arr, name, str, i))));
    }
    return list;
  }
